  ...
  const handlePayment = async () => {
    ...
    const options = {
      ...
      handler: function (response: any) {
        ...
        // Here you would typically verify the payment signature on your server and then grant access to the course
        setIsPaying(false);
      },
      prefill: {
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div>
            <Card className="glass-card">
              ...
              <CardContent>
                <p className="text-muted-foreground">You are about to purchase the course: <span className="font-bold text-foreground">"{course.title}"</span>. Click the button below to proceed to payment.</p>
              </CardContent>
              <CardFooter>
                <Button size="lg" className="w-full gradient-btn gradient-btn-1 relative" onClick={handlePayment} disabled={isPaying}>
                  {isPaying ? <Loader2 className="animate-spin" /> : `Pay ${getConvertedPrice(course.price)}`}
